...
/* loaded from: Jackey Client b2.jar:org/apache/log4j/LogMF.class */
            ...
            while (true) {
                ...
                if (pos >= 0) {
                    if (pos + 2 < pattern.length() && pattern.charAt(pos + 2) == '}' && pattern.charAt(pos + 1) >= '0' && pattern.charAt(pos + 1) <= '9') {
                        ...
                        prev = pos + 3;
                        indexOf = pattern.indexOf(123, prev);
                    } else {
                        indexOf = pattern.indexOf(123, pos + 1);
                    }
                } else {
            ...
            while (true) {
                ...
                if (pos >= 0) {
                    if (pos + 2 < pattern.length() && pattern.charAt(pos + 2) == '}' && pattern.charAt(pos + 1) >= '0' && pattern.charAt(pos + 1) <= '9') {
                        ...
                        String retval2 = new StringBuffer().append(retval).append(pattern.substring(prev, pos)).toString();
                        if (index != 0) {
                            str = new StringBuffer().append(retval2).append(pattern.substring(pos, pos + 3)).toString();
                        } else {
                            if (formatted == null) {
                                formatted = formatObject(arg0);
                            }
                            str = new StringBuffer().append(retval2).append(formatted).toString();
                        }
                        retval = str;
                        prev = pos + 3;
                        indexOf = pattern.indexOf(123, prev);
