...
/* loaded from: Jackey Client b2.jar:org/spongepowered/asm/util/throwables/SyntheticBridgeException.class */
public class SyntheticBridgeException extends MixinException {
    ...
    /* renamed from: a */
    private final AbstractInsnNode f444a;

    /* renamed from: b */
    private final AbstractInsnNode f445b;

    /* loaded from: Jackey Client b2.jar:org/spongepowered/asm/util/throwables/SyntheticBridgeException$Problem.class */
    public enum Problem {
    ...
    public void printAnalysis(IMixinContext context, MethodNode mda, MethodNode mdb) {
        ...
        printer.addWrapped(100, getMessage(), new Object[0]).m11hr();
        printer.add().m9kv("Method", this.name + this.desc).m9kv("Problem Type", this.problem).add().m11hr();
        ...
        printMethod(printer.add("Existing method").add().m9kv("Owner", owner).add(), mda).m11hr();
        printMethod(printer.add("Incoming method").add().m9kv("Owner", context.getClassRef().replace('/', '.')).add(), mdb).m11hr();
    ...
    private PrettyPrinter printMethod(PrettyPrinter printer, MethodNode method) {
        ...
        while (iter.hasNext()) {
            printer.m9kv(index == this.index ? ">>>>" : "", Bytecode.describeNode(iter.next()));
            index++;
        }
        return printer.add();
    }
    ...
    private PrettyPrinter printProblem(PrettyPrinter printer, IMixinContext context, MethodNode mda, MethodNode mdb) {
        ...
        switch (this.problem) {
            case BAD_INSN:
                ...
                printer.add("at index " + this.index + CallSiteDescriptor.TOKEN_DELIMITER).add();
                printer.m9kv("Existing opcode: %s", Bytecode.getOpcodeName(this.f444a));
                printer.m9kv("Incoming opcode: %s", Bytecode.getOpcodeName(this.f445b)).add();
            ...
            case BAD_LOAD:
                ...
                while (ia.hasNext() && ib.hasNext()) {
                    ...
                    if ((na instanceof VarInsnNode) && (nb instanceof VarInsnNode)) {
                        ...
                        printer.m8kv("Target " + index, "%8s %-2d %s", Bytecode.getOpcodeName(va), Integer.valueOf(va.var), ta);
                        printer.m8kv("Incoming " + index, "%8s %-2d %s", Bytecode.getOpcodeName(vb), Integer.valueOf(vb.var), tb);
                        if (ta.equals(tb)) {
                            printer.m8kv("", "Types match: %s", ta);
                        } else if (ta.getSort() != tb.getSort()) {
                            printer.m9kv("", "Types are incompatible");
                        } else if (ta.getSort() == 10) {
                            ClassInfo superClass = ClassInfo.getCommonSuperClassOrInterface(ta, tb);
                            printer.m8kv("", "Common supertype: %s", superClass);
                        }
                        printer.add();
                    }
                    index++;
            ...
            case BAD_CAST:
                ...
                Type ta2 = Type.getObjectType(((TypeInsnNode) this.f444a).desc);
                Type tb2 = Type.getObjectType(((TypeInsnNode) this.f445b).desc);
                printer.m9kv("Target type", ta2);
                printer.m9kv("Incoming type", tb2);
                printer.m9kv("Common supertype", ClassInfo.getCommonSuperClassOrInterface(ta2, tb2)).add();
                break;
            ...
            case BAD_INVOKE_DESC:
                MethodInsnNode mdna = (MethodInsnNode) this.f444a;
                MethodInsnNode mdnb = (MethodInsnNode) this.f445b;
    ...
    private PrettyPrinter printTypeComparison(PrettyPrinter printer, String index, Type tpa, Type tpb) {
        printer.m8kv("Target " + index, "%s", tpa);
        printer.m8kv("Incoming " + index, "%s", tpb);
        if (tpa.equals(tpb)) {
            printer.m8kv("Analysis", "Types match: %s", tpa);
        } else if (tpa.getSort() != tpb.getSort()) {
            printer.m9kv("Analysis", "Types are incompatible");
        } else if (tpa.getSort() == 10) {
            ClassInfo superClass = ClassInfo.getCommonSuperClassOrInterface(tpa, tpb);
            printer.m8kv("Analysis", "Common supertype: L%s;", superClass);
        }
        return printer.add();
    }
}
