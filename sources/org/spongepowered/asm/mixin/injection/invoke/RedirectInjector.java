...
/* loaded from: Jackey Client b2.jar:org/spongepowered/asm/mixin/injection/invoke/RedirectInjector.class */
public class RedirectInjector extends InvokeInjector {
    ...
    /* loaded from: Jackey Client b2.jar:org/spongepowered/asm/mixin/injection/invoke/RedirectInjector$Meta.class */
    public class Meta {
    ...
    /* loaded from: Jackey Client b2.jar:org/spongepowered/asm/mixin/injection/invoke/RedirectInjector$ConstructorRedirectData.class */
    ...
    /* loaded from: Jackey Client b2.jar:org/spongepowered/asm/mixin/injection/invoke/RedirectInjector$RedirectedInvoke.class */
    ...
    protected void validateParams(RedirectedInvoke invoke) {
        ...
        for (int index = 0; index < argc; index++) {
            if (index >= this.methodArgs.length) {
                throw new InvalidInjectionException(this.info, String.format("%s has an invalid signature. Not enough arguments found for capture of target method args, expected %d but found %d", description, Integer.valueOf(argc), Integer.valueOf(this.methodArgs.length)));
            }
            ...
            if (index < invoke.locals.length) {
                ...
                if (arg >= invoke.target.arguments.length) {
                    throw new InvalidInjectionException(this.info, String.format("%s has an invalid signature. Found unexpected additional target argument with type %s at index %d", description, fromType, Integer.valueOf(index)));
                }
            ...
            if (fromType.equals(toType)) {
                ...
                if (coerce == null) {
                    throw new InvalidInjectionException(this.info, String.format("%s has an invalid signature. Found unexpected argument type %s at index %d, expected %s", description, fromType, Integer.valueOf(index), toType));
                }
                if (!canCoerce) {
                    throw new InvalidInjectionException(this.info, String.format("%s has an invalid signature. Cannot @Coerce argument type %s at index %d to %s", description, toType, Integer.valueOf(index), fromType));
                }
            }
        }
    }
    ...
    private void injectAtArrayField(Target target, FieldInsnNode fieldNode, int opCode, Type ownerType, Type fieldType, int fuzz, int opcode) {
        ...
        AbstractInsnNode varNode2 = BeforeFieldAccess.findArrayNode(target.insns, fieldNode, elementType.getOpcode(79), fuzz);
        injectAtSetArray(target, fieldNode, varNode2, ownerType, fieldType);
    }
    ...
    private void injectAtSetArray(Target target, FieldInsnNode fieldNode, AbstractInsnNode varNode, Type ownerType, Type fieldType) {
        String handlerDesc = Bytecode.generateDescriptor(null, getArrayArgs(fieldType, 1, fieldType.getElementType()));
    ...
    public void injectArrayRedirect(Target target, FieldInsnNode fieldNode, AbstractInsnNode varNode, boolean withArgs, String type) {
        if (varNode == null) {
            throw new InvalidInjectionException(this.info, String.format("Array element %s on %s could not locate a matching %s instruction in %s. %s", this.annotationType, this, type, target, ""));
        }
        if (!this.isStatic) {
    ...
    private AbstractInsnNode injectAtPutField(InsnList insns, Target target, FieldInsnNode node, boolean staticField, Type owner, Type fieldType) {
        ...
        target.addToStack((this.isStatic || staticField) ? 0 : 1);
        return invokeHandler(insns);
    }

    public boolean checkDescriptor(String desc, Target target, String type) {
    ...
    protected void injectAtConstructor(Target target, InjectionNodes.InjectionNode node) {
        ...
        try {
            boolean withArgs = checkDescriptor(desc, target, "constructor");
            if (isAssigned) {
            ...
            if (isAssigned) {
                ...
                throwException(insns, "java/lang/NullPointerException", String.format("%s constructor handler %s returned null for %s", this.annotationType, this, newNode.desc.replace('/', '.')));
    ...
    private static String getGetArrayHandlerDescriptor(AbstractInsnNode varNode, Type returnType, Type fieldType) {
        if (varNode != null && varNode.getOpcode() == 190) {
            return Bytecode.generateDescriptor(Type.INT_TYPE, getArrayArgs(fieldType, 0, new Type[0]));
        }
        return Bytecode.generateDescriptor(returnType, getArrayArgs(fieldType, 1, new Type[0]));
    }
