...
/* loaded from: Jackey Client b2.jar:org/spongepowered/asm/mixin/injection/invoke/ModifyConstantInjector.class */
public class ModifyConstantInjector extends RedirectInjector {
    ...
    private void checkNarrowing(Target target, AbstractInsnNode constNode, Type constantType) {
        ...
        if (pop instanceof FieldInsnNode) {
            ...
            checkNarrowing(target, constNode, constantType, fieldType, target.indexOf(pop), String.format("%s %s %s.%s", Bytecode.getOpcodeName(pop), SignaturePrinter.getTypeName(fieldType, false), fieldNode.owner.replace('/', '.'), fieldNode.name));
        } else if (pop.getOpcode() == 172) {
            checkNarrowing(target, constNode, constantType, target.returnType, target.indexOf(pop), "RETURN " + SignaturePrinter.getTypeName(target.returnType, false));
            ...
            checkNarrowing(target, constNode, constantType, localType, target.indexOf(pop), String.format("ISTORE[var=%d] %s %s", Integer.valueOf(var), SignaturePrinter.getTypeName(localType, false), name));
        }
    }
