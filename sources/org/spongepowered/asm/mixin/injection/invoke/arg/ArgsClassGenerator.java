...
/* loaded from: Jackey Client b2.jar:org/spongepowered/asm/mixin/injection/invoke/arg/ArgsClassGenerator.class */
public final class ArgsClassGenerator implements IClassGenerator {
    ...
    public String getClassName(String desc) {
        ...
        if (name == null) {
            ...
            this.nextIndex = i + 1;
            name = String.format("%s%d", CLASS_NAME_BASE, Integer.valueOf(i));
    ...
    private byte[] generateClass(String name, String desc) {
        ...
        generateCtor(ref, desc, args, visitor);
        generateToString(ref, desc, args, visitor);
        generateFactory(ref, desc, args, visitor);
        generateSetters(ref, desc, args, visitor);
        generateGetters(ref, desc, args, visitor);
        visitor.visitEnd();
    ...
    private void generateIndexedSetter(String ref, String desc, Type[] args, ClassVisitor writer) {
        ...
        for (int index2 = 0; index2 < args.length; index2++) {
            String boxingType = Bytecode.getBoxingType(args[index2]);
