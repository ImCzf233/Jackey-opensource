...
/* loaded from: Jackey Client b2.jar:com/viaversion/viaversion/util/CompactArrayUtil.class */
public class CompactArrayUtil {
    ...
    public static long[] createCompactArrayWithPadding(int bitsPerEntry, int entries, IntToLongFunction valueGetter) {
        long maxEntryValue = (1 << bitsPerEntry) - 1;
        ...
        for (int i = 0; i < entries; i++) {
            ...
            data[cellIndex] = (data[cellIndex] & ((maxEntryValue << bitIndex) ^ (-1))) | ((value & maxEntryValue) << bitIndex);
        }
        return data;
    }
    ...
    public static long[] createCompactArray(int bitsPerEntry, int entries, IntToLongFunction valueGetter) {
        ...
        for (int i = 0; i < entries; i++) {
            ...
            data[startIndex] = (data[startIndex] & ((maxEntryValue << startBitSubIndex) ^ (-1))) | ((value & maxEntryValue) << startBitSubIndex);
    ...
    public static void iterateCompactArray(int bitsPerEntry, int entries, long[] data, BiIntConsumer consumer) {
        ...
        for (int i = 0; i < entries; i++) {
            ...
            int startBitSubIndex = bitIndex % 64;
            if (startIndex == endIndex) {
                j = data[startIndex] >>> startBitSubIndex;
            } else {
                int endBitSubIndex = 64 - startBitSubIndex;
                j = (data[startIndex] >>> startBitSubIndex) | (data[endIndex] << endBitSubIndex);
            }
            int value = (int) (j & maxEntryValue);
            consumer.consume(i, value);
        }
    }
}
